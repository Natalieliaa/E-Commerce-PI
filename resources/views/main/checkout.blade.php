...
@section('content')
    ...
    <div class="max-w-4xl mx-auto p-8 bg-white rounded-xl shadow-lg mt-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mb-6 flex items-center">
            <svg class="w-8 h-8 mr-3 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        ...
        <form method="POST" action="{{ route('checkout.submit') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 border-b pb-6">
                ...
                <div class="flex items-start p-3 border border-gray-200 rounded-lg bg-gray-50">
                    <img src="{{ asset('images/' . $item['image']) }}" alt="{{ $item['name'] }}" class="w-20 h-20 object-cover rounded-md shadow-sm mr-3">
                    <div class="flex-grow">
                        ...
                        <p class="text-md font-medium text-gray-700">Harga: Rp {{ number_format($item['price'], 0, ',', '.') }}</p>
            ...
            <div class="mb-8">
                {{-- Menggunakan textarea atau select/input untuk alamat --}}
            ...
            <div class="mb-8">
                <select name="payment_method"
                        class="w-full p-3 border border-gray-400 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-600 appearance-none bg-white" required>
            ...
            {{-- =================================== --}}
            {{-- 4. Catatan untuk Penjual --}}
            {{-- =================================== --}}
            <h2 class="text-2xl font-semibold text-gray-800 mb-4">Catatan untuk Penjual</h2>
            <div class="mb-10">
                <textarea name="seller_notes" rows="3"
                          class="w-full p-3 border border-gray-400 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-600"
                          placeholder="Opsional: Tambahkan catatan khusus untuk penjual, misalnya permintaan bungkus kado atau pesan khusus.">Tolong ditambahkan tulisan "Selamat ulang tahun ibu"</textarea>
            </div>

            {{-- =================================== --}}
            {{-- 5. Total Pembayaran & Tombol --}}
            {{-- =================================== --}}
            <div class="flex justify-between items-center pt-4 border-t border-gray-300">
                <span class="text-2xl font-extrabold text-gray-900">Total Pembayaran: Rp {{ number_format($totalPrice, 0, ',', '.') }}</span>
...
@push('scripts')
<script>
    // Script opsional untuk validasi atau interaksi front-end saat checkout
    document.addEventListener('DOMContentLoaded', function() {
        // Contoh: Ambil alamat default pengguna jika ada
        const defaultAddress = "{{ Auth::user() ? Auth::user()->default_address : '' }}";
        const addressTextarea = document.querySelector('textarea[name="shipping_address"]');

        if (addressTextarea.value.trim() === '' && defaultAddress) {
